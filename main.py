...
class Matrix:
    def __init__(self, matrix_data: List[List]):
        ...
        self._matrix_size = [(matrix_rows, len(row)) for row in self._matrix]

    def __add__(self, other: "Matrix") -> "Matrix":
        if not isinstance(other, Matrix):
            raise TypeError(f"Не является матрицей")
        if self._matrix_size != other._matrix_size:
            raise ValueError(f"Отличия в размерах матриц")

        result = []

        for row in zip(self._matrix, other._matrix):
            result.append([sum([i, j]) for i, j in zip(*row)])
...
matrix_one = Matrix([[1, 3, 5], [2, 4, 6]])
matrix_two = Matrix([[5, 3, 1], [6, 4, 2]])
temp = matrix_one + matrix_two
print(temp)
print(type(temp))
print()
